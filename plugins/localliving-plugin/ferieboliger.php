...
echo '
    ...
    jQuery(document).ready(function ($) {
       ...
       $(".pause-auto-update-toggle-all").parent().on("click", function() {
           var selector = $(this).find(".pause-auto-update-toggle-all")[0];
           var toggleStatus = $(selector).prop("checked") ? "off" : "on";
           var supplierId = $(selector).val();
           
           $(this).parents(".supplier-wrapper").find("input.pause-auto-update-toggle-accommodation").bootstrapToggle(toggleStatus);
           ...
           $.ajax({
                ...
                url: "' . admin_url("admin-ajax.php") . '",
                data: {
                   action: "toggle_pause_auto_update",
                   supplierId: supplierId,
                   accommodationId: "",
                   mode: toggleStatus
                },
       ...
       $(".pause-auto-update-toggle-accommodation").parent().on("click", function() {
           var selector = $(this).find(".pause-auto-update-toggle-accommodation")[0];
           var toggleStatus = $(selector).prop("checked") ? "off" : "on";
           var accommodationId = $(selector).val();
...
if(!isset($_SESSION['cpage']) && !isset($_POST['cpage'])) {
	$_SESSION['cpage'] = 1;
}

if(isset($_POST['cpage'])) {
	$_SESSION['cpage'] = $_POST['cpage'];
}

$generateFullCsvByYearAction = $_POST['GenerateFullCsvByYear'] ?? '';
$yearGenerateFullCsv         = $_POST['YearGenerateFullCsv']   ?? date("Y");

if($generateFullCsvByYearAction == 'GenerateFullCsvByYear') {
	$downloadFileName = "auto_upload_itravel_$yearGenerateFullCsv.csv";
    $downloadFileUrl  = get_site_url() . "/wp-content/plugins/localliving-plugin/csv_logs/$downloadFileName";
    
    if(is_file(CSV_LOGS.$downloadFileName)) {
	    echo '<script>window.open("'.$downloadFileUrl.'","_blank")</script>';
    } else {
	    echo '<script>alert("No CSV file detected! Please try again...")</script>';
    }
}
...
function getSuppliersList(
    ...
    if($viewExceptionOnly == '1') {
        ...
        $query .= " WHERE ($table_name_accommodations.accommodation_exception_status = 0
        OR $table_name_accommodations.accommodation_exception_status IS NULL)";
    }
...
//        )";
    ...
    if($groupBySupplierId) {
        $query = " SELECT q.* FROM ($query) q GROUP BY q.supplier_id ORDER BY q.supplier_name";
    }
...
//supplier options list is missing some
function getAllSupplierOptions() {
    ...
    $query = "SELECT *
    FROM $table_name_suppliers
    WHERE $table_name_suppliers.supplier_exception_status = 0
    ";
...
function isSupplierException($supplierId) {
    ...
    $query = "SELECT $table_name_suppliers.supplier_exception_status
            FROM $table_name_suppliers
            WHERE $table_name_suppliers.supplier_id = $supplierId";
...
function isSupplierAutoUpdatePause($supplierId) {
    global $wpdb;
    ...
    $query = "SELECT $table_name_accommodation.accommodation_auto_update_pause
    ...
    foreach ($queryResults as $queryResult) {
        if($queryResult->accommodation_auto_update_pause == '0') {
            $result = false;
            break;
        }
    }
...
function isAccommodationAutoUpdatePause($accommodationId) {
    global $wpdb;
    ...
    $query = "SELECT $table_name_accommodation.accommodation_auto_update_pause
    FROM $table_name_accommodation
    WHERE $table_name_accommodation.accommodation_id = $accommodationId
    ";
...
function renderPagination($dataTotal, $perPage, $currentPage)
{
    ...
    echo '<li class="pagination-input">';
    echo '<input type="number" min="1" max="'.$numberOfPages.'" name="cpage" value="'.$currentPage.'" form="ferieboliger-search-form"/>';
    echo '<button type="submit" form="ferieboliger-search-form"> › </button>';
    echo '</li>';
    
    echo '<li class="pagination-buttons">';
    
    if ($currentPage <= 1) {
        echo '<button disabled="disabled">«</button>';
        echo '<button disabled="disabled">‹</button>';
    } else {
        echo '<button type="submit" name="cpage" value="1" form="ferieboliger-search-form">«</button>';
        echo '<button type="submit" name="cpage" value="' . ($currentPage - 1) . '" form="ferieboliger-search-form">‹</button>';
    }
    ...
    if ($currentPage >= $numberOfPages) {
        ...
        echo '<button type="submit" name="cpage" value="' . ($currentPage + 1) . '" form="ferieboliger-search-form">›</button>';
        echo '<button type="submit" name="cpage" value="' . $numberOfPages . '" form="ferieboliger-search-form">»</button>';
    }
...
function groupAccommodationWithSuppliers() {
    ...
    $itemsPerPage = 5;
    $currentPage = isset($_SESSION['cpage']) ? abs((int)$_SESSION['cpage']) : 1;
...
function renderSuppliersOptions() {
    $suppliersList = getAllSupplierOptions();
...
function renderResultByGroupSupplier() {
    ...
    foreach ($accommodationsGroupBySupplierList as $supplierId => $accommodationsList) {
        $supplierName = getSupplierDetailById($supplierId)->supplier_name ?? '';
        ...
        echo '</th>';
        echo '<th width="25%"></th>';
        echo '<th width="15%" align="right"><input class="toggle-input pause-auto-update-toggle-all"
                    name="PauseAutoUpdateBySupplier" value="'.$supplierId.'" type="checkbox" data-toggle="toggle"';
        ...
        if(isSupplierAutoUpdatePause($supplierId)) {
            echo ' checked="checked"';
        }
        ...
        foreach ($accommodationsList as $accommodation) {
            ...
            echo '<input class="toggle-input pause-auto-update-toggle-accommodation"
                    name="PauseAutoUpdateByAccommodation" value="'.$accommodation->accommodation_id.'"';
            ...
            if(isAccommodationAutoUpdatePause($accommodation->accommodation_id)) {
                echo ' checked="checked"';
            }
            
            echo ' type="checkbox" data-toggle="toggle">';
            echo '</td>';
            echo '<td valign="middle" align="right">
...
function renderResultByList() {
    echo '<tr>';
    echo '<td class="table-child-wrapper" colspan="6">';
    echo '<form method="POST" action="'.admin_url( 'admin.php' ) . '?page=rediger_feriebolig'.'">';
...
<div class="localliving-ferieboliger" style="display: none;">
    ...
    <div id="ferieboliger">
        <form id="ferieboliger-search-form" name="FerieboligerSearchForm" method="POST">
            ...
            <div class="above-result-list-wrapper">
                ...
                <div class="download-full-csv">
                    <input type="number" name="YearGenerateFullCsv" value="<?php echo date("Y"); ?>"/>
                    <button id="generate-full-csv-btn" type="submit" class="btn btn-primary" name="GenerateFullCsvByYear" value="GenerateFullCsvByYear">
                        Download Full CSV By Year</button>
                </div>
                            ...
                            if (isset($_POST['ViewGroupAfterSupplier'])) {
                                ...
                                if($viewGroupAfterSupplier == '1') {
                                    echo '
                                        ...
                                        <th align="right">Pause auto update</th>
                                    ...
                                    echo '
                                        ...
                                        <th width="24%">Senest opdateret</th>
                                        <th width="17%" align="right">Pause auto update</th>
