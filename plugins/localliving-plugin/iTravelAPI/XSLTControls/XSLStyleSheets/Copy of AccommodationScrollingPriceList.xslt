...
>
	...
	<xsl:template name="DrawTableRowBasicScroll">
		...
		<xsl:variable name="minPersons">
			<xsl:variable name="minPersons">
				<xsl:for-each select="PriceRowList/PriceRow[last()]">
					<xsl:choose>
						<xsl:when test="position()=last()">
							<xsl:value-of select="../../../../AttributeGroupList/AttributeGroup/AttributeList/Attribute[AttributeID=120]/AttributeValue" />
						</xsl:when>
		...
		<xsl:choose>
			<xsl:when test="$minPersons='0' and $objectHasShortStayNote = '1'">
				<xsl:text>1</xsl:text>
			</xsl:when>
			<xsl:otherwise>
		...
		<xsl:for-each select="PriceRowList/PriceRow">
			...
			<xsl:if test="not($minPersons='0' and $objectHasShortStayNote = '1')">
				<tr data-has="{$objectHasShortStayNote}" data-min="{$minPersons}">
					<!--If left side of table is visible -->
					<xsl:if test="$ShowLeftSide = true()">
						<!-- Output "unit name - service name"-->
						<xsl:if test="position() = 1 or( $nameCol!='' and position()=2)">
							<xsl:variable name="rowSpan" select="count(../PriceRow) - string-length($hiddenRowsCount)"/>
