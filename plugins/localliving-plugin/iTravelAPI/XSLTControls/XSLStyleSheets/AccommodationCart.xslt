            ...
            <div class="article-wrapper {$region-hor-line-class}">
              <article class="row search-results-row">
                ...
                <div class="col-md-2 last-column">
                  ...
                  <button class="remove-from-cart" data-object-id="{$objectID}">Slet</button>
                </div>
              </article>
                      ...
                      <div class="row">
                        ...
                        <div class="col-md-1 villa-unit-image">
                          <xsl:choose>
                            ...
                            <xsl:otherwise>
                            <a class="unit-gallery transition-slow" title="">
                              <img class="lazy"
                        ...
                        <div class="col-md-6 last-column">
                          <div class="row row-reverse">
                            <div class="col-md-4 remove-from-cart-wrapper">
                              <button class="remove-from-cart" data-object-id="{$objectID}" data-unit-id="{UnitID}">Slet</button>
                            </div>
                            <div class="villa-unit-price-row col-md-8">
                                ...
                                <div class="discount">
                                  ...
                                  <xsl:for-each
                                    ...
                                    <xsl:variable name="serviceID"
                                                  select="ServiceID"/>
                                    <xsl:value-of
                                        select="../../../SpecialOfferList/SpecialOffer[ServiceID=$serviceID]/ServiceName"/>
                              ...
                              <xsl:if test="$BootstrapCSSClass='col-md-4'">
                                <div class="col-md-3 text-right book-button-holder">
                                  ...
                                  <!--                                                  &lt;!&ndash; Booking addres link - address defined above! &ndash;&gt;-->
                                  <!--                                                  <xsl:choose>-->
          ...
          <xsl:when test="$showWishListButton=1">
            <div class="no-results">
              <h2 class="no-results-naslov">
                <xsl:value-of
                    select="/*/TranslationList/Translation[LanguageID=$languageID]/root/data[@name='noResultsFound']/value"
              ...
              <p class="no-results-review">
                ...
                <br/>
                <xsl:value-of
                    select="/*/TranslationList/Translation[LanguageID=$languageID]/root/data[@name='noFavoritesFound']/value"
                    disable-output-escaping="yes"/>
              </p>
            </div>
          </xsl:when>
          <xsl:otherwise>
            <div class="no-results col-md-9 col-md-offset-3">
              <xsl:if test="$beginNextWeek != 'false'">
                <table class="checkAnotherWeekContainer">
                  <tr class="checkAnotherWeekTop">
                    <td class="imageCell" align="center">
                      <img class="lazy" data-src="/wp-content/themes/localliving/images/no-results.png"/>
                    </td>
                    <td class="textCell">
                      <h2 class="uppercase">
                        <xsl:value-of
                            select="/*/TranslationList/Translation[LanguageID=$languageID]/root/data[@name='weFoundNoResults']/value"/>
                      </h2>
                      <br/>
                      <xsl:value-of
                          select="/*/TranslationList/Translation[LanguageID=$languageID]/root/data[@name='checkOtherDates']/value"/>
                      <xsl:text>:</xsl:text>
                    </td>
                  </tr>
                </table>
                <table class="checkAnotherWeekContainer">
                  <tr class="checkAnotherWeekBottom">
                    <xsl:variable name="nextLink">
                      <xsl:call-template name="paginationUrl">
                        <xsl:with-param name="pageNumberParameter" select="1"/>
                        <xsl:with-param name="overrideFrom" select="$beginNextWeek"/>
                        <xsl:with-param name="overrideTo" select="$endNextWeek"/>
                      </xsl:call-template>
                    </xsl:variable>
                    <xsl:if test="$beginPreviousWeek != ''">
                      <xsl:variable name="previousLink">
                        <xsl:call-template name="paginationUrl">
                          <xsl:with-param name="pageNumberParameter" select="1"/>
                          <xsl:with-param name="overrideFrom" select="$beginPreviousWeek"/>
                          <xsl:with-param name="overrideTo" select="$endPreviousWeek"/>
                        </xsl:call-template>
                      </xsl:variable>
                      <td class="previousLink">
                        <a href="{$previousLink}">
                          <div style="text-align: left;">
                            <span class="online-booking-page">
                              <xsl:value-of
                                  select="/*/TranslationList/Translation[LanguageID=$languageID]/root/data[@name='weekBefore']/value"/>
                            </span>
                            <br/>
                            <xsl:call-template name="formatDate">
                              <xsl:with-param name="date" select="$beginPreviousWeekDate"/>
                              <xsl:with-param name="format" select="$dateFormat"/>
                            </xsl:call-template>
                            <xsl:text> </xsl:text>
                            <span class="lowercase">
                              <xsl:value-of
                                  select="/*/TranslationList/Translation[LanguageID=$languageID]/root/data[@name='till']/value"/>
                            </span>
                            <xsl:text> </xsl:text>
                            <xsl:call-template name="formatDate">
                              <xsl:with-param name="date" select="$endPreviousWeekDate"/>
                              <xsl:with-param name="format" select="$dateFormat"/>
                            </xsl:call-template>
                          </div>
                        </a>
                    ...
                    <td class="nextLink">
                      <a href="{$nextLink}">
                        <div style="text-align: left;">
                          <span class="online-booking-page">
                            <xsl:value-of
                                select="/*/TranslationList/Translation[LanguageID=$languageID]/root/data[@name='weekAfter']/value"/>
                          </span>
                          <br/>
                          <xsl:call-template name="formatDate">
                            <xsl:with-param name="date" select="$beginNextWeekDate"/>
                            <xsl:with-param name="format" select="$dateFormat"/>
                          </xsl:call-template>
                          <xsl:text> </xsl:text>
                          <span class="lowercase">
                          ...
                          <xsl:text> </xsl:text>
                          <xsl:call-template name="formatDate">
                            <xsl:with-param name="date" select="$endNextWeekDate"/>
                            <xsl:with-param name="format" select="$dateFormat"/>
                          </xsl:call-template>
                        </div>
                      </a>
